...
function countPizzasInToppings(pizzas) {
    ...
    const counts =pizzas.map(pizza => pizza.toppings).flat().reduce((acc, currentTopping) => {
        // check if this is an existing topping
        const existingTopping = acc[currentTopping.id];
        if(existingTopping) {
            // if it is, add to the count (increment by 1)
            existingTopping.count += 1;
        } else {
            // otherwise create a new entry in the accumulator and set it to 1
